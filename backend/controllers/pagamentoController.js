...
async function pagarmeRequest(method, endpoint, body = null) {
  ...
  if (!secretKey) {
    throw new Error('PAGARME_SECRET_KEY nao configurada no .env');
  }
...
function verificarAssinaturaWebhook(payload, assinaturaRecebida) {
  ...
  if (!secret) {
    console.warn('[WEBHOOK] PAGARME_WEBHOOK_SECRET nao configurado');
    return true;
  }
...
const criarTaxaPlataforma = async (empresa_id, vaga_id, titulo_vaga) => {
  try {
    ...
    const descricao = `Taxa de publicacao: ${titulo_vaga}`;
...
const criarCobrancaBolsa = async (empresa_id, vaga_id, valor_bolsa, candidato_id) => {
  try {
    const descricao = `Bolsa tecnica - Candidato: ${candidato_id}`;
...
const receberWebhook = async (req, res) => {
  try {
    ...
    console.log(`[WEBHOOK] Evento: ${type}`, data?.id);

    if (type === 'order.paid' || type === 'charge.paid') {
      const orderId = data?.order_id || data?.id;

      if (!orderId) {
        return res.status(400).json({ erro: 'order_id nao encontrado no evento' });
      }
      ...
      if (resultado.changes > 0) {
        ...
        console.warn(`[WEBHOOK] Nenhum pagamento pendente para: ${orderId}`);
      }
    }
...
const listarPagamentos = async (req, res) => {
  try {
    ...
    if (!empresa_id) return res.status(400).json({ erro: 'empresa_id e obrigatorio' });
...
const obterLinkPagamento = async (req, res) => {
  try {
    ...
    if (pagamento.status === 'pago') {
      return res.status(200).json({ status: 'pago', mensagem: 'Este pagamento ja foi realizado.' });
    }
...
const obterMetricas = async (req, res) => {
  try {
    ...
    res.status(500).json({ erro: 'Erro ao obter metricas' });
  }
};
...
const obterMetricasVaga = async (req, res) => {
  try {
    ...
    if (!vaga) return res.status(404).json({ erro: 'Vaga nao encontrada' });
    ...
    res.status(500).json({ erro: 'Erro ao obter metricas da vaga' });
  }
};
